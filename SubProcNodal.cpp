    ...
    switch (LOWORD(wParam)) {
      ...
      case ID_OP4:
        if (wPrvKeyDwn != ID_OP4) {
          ...
          app.RubberBandingDisable();
          if (rPrvPos != rCurPos) {
            rCurPos = UserAxisSnapLn(rPrvPos, rCurPos);
  ...
  while (pos != nullptr) {
    ...
    while (posPrim != nullptr) {
      ...
      for (int i = 0; i < (int)pts.GetSize(); i++) {
        if (p2DPtInWnd(pts[i], rWnd)) nodal::UpdLis(nodal::bAdd, pSeg, pPrim, dwMask, i, pts[i]);
      }
    }
  }
}
...
void nodal::AddByPt(const CPnt& pt) {
  ...
  while (pos != nullptr) {
    ...
    while (posPrim != nullptr) {
      ...
      for (int i = 0; i < (int)pts.GetSize(); i++) {
        if (CVec(pts[i], pt).Length() <= .05) nodal::UpdLis(nodal::bAdd, pSeg, pPrim, dwMask, i, pts[i]);
      }
    }
  }
}
...
void nodal::AddPrimByPt(const CPnt pt) {
  ...
  for (int i = 0; i < pts.GetSize(); i++) nodal::UpdLis(nodal::bAdd, pSeg, pPrim, dwMask, i, pts[i]);
}
...
void nodal::UpdLis(bool abAdd, CSeg* pSeg, CPrim* pPrim, DWORD dwMask, int aiBit, CPnt arPt) {
  ...
  if (abAdd)  // Adding points to list
  {
    ...
    {
      if (!nodal::SegList->Find(pSeg)) nodal::SegList->AddTail(pSeg);
