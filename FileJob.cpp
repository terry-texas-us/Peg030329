...
CPrimArc::CPrimArc(char* p, int iVer) {
  if (iVer == 1) {
    ...
    if (m_dSwpAng < 0.) {
      pt[0] =
          (m_ptCenter[0] + ((ptBeg[0] - m_ptCenter[0]) * cos(m_dSwpAng) - (ptBeg[1] - m_ptCenter[1]) * sin(m_dSwpAng)));
      pt[1] =
          (m_ptCenter[1] + ((ptBeg[0] - m_ptCenter[0]) * sin(m_dSwpAng) + (ptBeg[1] - m_ptCenter[1]) * cos(m_dSwpAng)));
    } else {
      pt = ptBeg;
    }
    ...
    if (m_dSwpAng > TWOPI || m_dSwpAng < -TWOPI) m_dSwpAng = TWOPI;
  }
}
...
void CPrimDim::Write(CFile& f, char* p) const {
  ...
  *pChrs = short(m_strText.GetLength());
  ...
  for (int i = 0; i < *pChrs; i++) pChr[i] = m_strText[i];
...
CPrimText::CPrimText(char* p, int iVer) {
  ...
  if (iVer == 1) {
    ...
    if (m_fd.TextPath() < 0 || m_fd.TextPath() > 4) m_fd.TextPathSet(CFontDef::PATH_RIGHT);
    m_fd.TextHorAlignSet((WORD)fmod(d / 10., 10.));
    if (m_fd.TextHorAlign() < 1 || m_fd.TextHorAlign() > 3) m_fd.TextHorAlignSet(CFontDef::HOR_ALIGN_CENTER);
    m_fd.TextVerAlignSet((WORD)(d / 100.));
    if (m_fd.TextVerAlign() < 2 || m_fd.TextVerAlign() > 4) m_fd.TextVerAlignSet(CFontDef::VER_ALIGN_MIDDLE);
...
void CPrimText::Write(CFile& f, char* p) const {
  ...
  *pChrs = short(m_strText.GetLength());
  ...
  for (int n = 0; n < m_strText.GetLength(); n++) pChr[n] = m_strText[n];
