...
double dArrowSiz = 0.1;     // Arrow size
      ...
      switch (LOWORD(wParam)) {
        ...
        case ID_OP3:  // Generate line with arrow head
          ...
          } else  // Operation pending
          {
            app.RubberBandingDisable();
            dD12 = CVec(pt[0], pt[1]).Length();
            if (dD12 > DBL_EPSILON) {  // Two points do not coincide
              pSeg = new CSeg;
              if (wPrvKeyDwn == ID_OP3) {
                annotate::GenArrowHead(annotate::iArrowTyp, annotate::dArrowSiz, pt[1], pt[0], pSeg);
                pSeg->AddTail(new CPrimLine(1, 1, pt[0], pt[1]));
        ...
        case ID_OP4:  // Generate a bubble
        {
          ...
          if (annotate::wBubFacets == 0) {  // Bubble is a circle
            pSeg->AddTail(new CPrimArc(1, 1, pt[1], vXAx, vYAx, TWOPI));
          } else {  // Bubble is a polygon
            int iPrimState = pstate.Save();
        ...
        case ID_OP7:
          ...
          if (wPrvKeyDwn != ID_OP7) {
            ...
            if (bG1Flg && bG2Flg) {
              ...
              for (i = 1; i < 4; i++) {
                pt[0][0] = std::min(pt[0][0], ptsBox1[i][0]);
                pt[2][0] = std::max(pt[2][0], ptsBox1[i][0]);
                pt[0][1] = std::min(pt[0][1], ptsBox1[i][1]);
                pt[2][1] = std::max(pt[2][1], ptsBox1[i][1]);
              }
              for (i = 0; i < 4; i++) {
                pt[0][0] = std::min(pt[0][0], ptsBox2[i][0]);
                pt[2][0] = std::max(pt[2][0], ptsBox2[i][0]);
                pt[0][1] = std::min(pt[0][1], ptsBox2[i][1]);
                pt[2][1] = std::max(pt[2][1], ptsBox2[i][1]);
              }
              pt[1][0] = pt[2][0];
              pt[1][1] = pt[0][1];
              pt[3][0] = pt[0][0];
...
void annotate::GenArrowHead(int aiArrowTyp, double adArrowSiz, const CPnt& ptTail, const CPnt& ptHead, CSeg* pSeg) {
  ...
  if (aiArrowTyp == 1 || aiArrowTyp == 2) {
    ...
    if (aiArrowTyp == 2) pSeg->AddTail(new CPrimLine(1, 1, pt4, pt5));
