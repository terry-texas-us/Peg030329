...
LRESULT CALLBACK SubProcFixup(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam) noexcept {
  static WORD wPrvKeyDwn{0};

  static CSeg* pSegPrv{nullptr};
  static CSeg* pSegRef{nullptr};
  static CSeg* pSegSec{nullptr};
  static CPrim* pPrimPrv{nullptr};
  static CPrim* pPrimRef{nullptr};
  static CPrim* pPrimSec{nullptr};
      ...
      case ID_OP1:
        ...
        if (wPrvKeyDwn == 0)
          ...
          if (wPrvKeyDwn == ID_OP2) {
            ...
            lnPrv[1] = ptInt;
            if (CVec(lnRef[1], ptInt).Length() < CVec(lnRef[0], ptInt).Length()) lnRef[1] = lnRef[0];
            lnRef[0] = ptInt;
      ...
      case ID_OP2:
        ...
        if (wPrvKeyDwn == 0) {
          ...
          if (wPrvKeyDwn == ID_OP2) {
            ...
            if (CVec(lnPrv[0], ptInt).Length() < CVec(lnPrv[1], ptInt).Length()) lnPrv[0] = lnPrv[1];
            lnPrv[1] = ptInt;
            if (CVec(lnSec[1], ptInt).Length() < CVec(lnSec[0], ptInt).Length()) lnSec[1] = lnSec[0];
            lnSec[0] = ptInt;
