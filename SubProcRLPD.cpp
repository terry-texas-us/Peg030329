  ...
  if (anMsg == WM_COMMAND) {
    ...
    switch (LOWORD(wParam)) {
      ...
      case ID_OP2:
        ...
        if (pEndCapSeg == nullptr) {  // No end-cap consideration
          if (wPrvKeyDwn == ID_OP2) {
            ...
            lnPar[1] = lnPar[3];
            lnLead(ptPrv, ptCur);
            if (ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2])) {  // Lead line vector is not null
              if (rlpd::bContSec) {
                if (ParLines_CleanCorners(&lnPar[0], &lnPar[2])) {
                  rlpd::GenElbow(dDiam[2], dDiam[2], lnPar);

                  if (!rlpd::bContSec) return 0;
                } else {
                  ...
                  ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2]);

                  pDoc->UpdateAllViews(nullptr, CPegDoc::HINT_SEG_ERASE_SAFE, pgEndCapSeg);
                  pDoc->AnyLayerRemove(pgEndCapSeg);
                  detsegs.Remove(pgEndCapSeg);
                  if (trapsegs.Remove(pgEndCapSeg) != nullptr) { app.StatusLineDisplay(TrapCnt); }
            ...
            lnLead(ptPrv, ptCur);
            if (ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2])) {  // Lead line vector is not null
              rlpd::GenTransition(rlpd::Begin, iJus, dSlo, dDiam, dDiam, &lnPar[2]);
      ...
      case ID_OP3: {
        ...
        if (pEndCapMark == nullptr)  // No end-cap consideration
        {
          if (wPrvKeyDwn == 0)  // Beginning initial section
          {
            ...
            lnPar[1] = lnPar[3];
            lnLead(ptPrv, ptCur);
            if (ParLines_GenPts(.5, dDiam[1], lnLead, &lnPar[2]))  // Lead line vector is not nu
            {
              CPnt ptCent[4];
              ...
              if (rlpd::bContSec) {
                if (ParLines_CleanCorners(&lnPar[0], &lnPar[2])) {
                  rlpd::GenElbow(dDiam[1], 0, lnPar);
                  if (!rlpd::bContSec) return 0;
                } else {
                  ptPrv[0] = (lnPar[0][0][0] + lnPar[1][0][0]) * .5f;
                  ptPrv[1] = (lnPar[0][0][1] + lnPar[1][0][1]) * .5f;
                  lnLead(ptPrv, ptCur);
                  ParLines_GenPts(.5, dDiam[1], lnLead, &lnPar[2]);
                  pDoc->UpdateAllViews(nullptr, CPegDoc::HINT_SEG_ERASE_SAFE, pgEndCapSeg);
                  pDoc->AnyLayerRemove(pgEndCapSeg);
                  detsegs.Remove(pgEndCapSeg);
                  if (trapsegs.Remove(pgEndCapSeg) != nullptr) { app.StatusLineDisplay(TrapCnt); }
              ...
              rlpd::GenTransition(rlpd::End, &iJus[1], dSlo, &dDiam[1], &dDiam[1], &lnPar[2]);
            ...
            lnPar[1] = lnPar[3];
            lnLead(ptPrv, ptCur);
            if (ParLines_GenPts(.5, dDiam[0], lnLead, &lnPar[2]))  // Lead line vector is not nu
            {
              rlpd::GenTransition(rlpd::Both, iJus, dSlo, dDiam, dDiam, &lnPar[2]);
              dDiam[1] = dDiam[2];
              rlpd::bContSec = false;
              pgEndCapSeg = nullptr;
              wPrvKeyDwn = ID_OP2;
        ...
        ptCur = Pnt_ProjPtTo(ptCur, ptTemp, rlpd::dInsideRadiusFactor * dDiam[2]);
      ...
      case ID_OP4: {
        ...
        if (!detsegs.SelSegAndPrimUsingPoint(ptCur)) break;
        if (wPrvKeyDwn != 0) app.RubberBandingDisable();
        ...
        if (pEndCapMark == nullptr)  // No end-cap consideration
        {
          if (wPrvKeyDwn == ID_OP2) {
            ...
            if (Pnt_DistanceTo_xy(lnLead[0], lnLead[1]) <= dDiam[2]) {
              wPrvKeyDwn = 0;
              break;
            }

            if (ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2]))  // Lead line vector is not nu
            {
              ptCent[0] = lnPar[2][0];
              ptCent[1] = lnPar[2][1];
              ptCent[2] = lnPar[3][0];
              ...
              dDiam[2] *= 2;
              dSlo[0] = dSlo[1] /= 12;
              rlpd::GenTransition(rlpd::End, &iJus[1], dSlo, &dDiam[1], &dDiam[2], &lnPar[2]);
              ptCur[0] = (lnPar[2][1][0] + lnPar[3][1][0]) * .5f;
              ptCur[1] = (lnPar[2][1][1] + lnPar[3][1][1]) * .5f;
              rlpd::bContSec = false;
              pgEndCapSeg = nullptr;
        ...
        if (Pnt_DistanceTo_xy(pt[1], lnPar[3][0]) < Pnt_DistanceTo_xy(pt[2], lnPar[3][0])) pt[1] = pt[2];
        pt[1] = Pnt_ProjPtTo(ptCur, pt[1], dDiam[2] * 0.25);
        pt[2] = lnPar[2][1];
        auto* pArc = new CPrimArc(pt[0], pt[1], pt[2]);
        pSeg->AddTail(pArc);

        cut::CutPrimsAtPt(pt[0]);
        cut::CutPrimsAtPt(pt[2]);
        pt[1] = Pnt_ProjPtTo(pt[0], pt[2], dDiam[2] * 0.25);
        pDoc->DoPrimDelete(pt[1]);

        if (rlpd::bGenCenterline) {
          ptCent[0] = Pnt_ProjPtTo(ptCent[0], ptCent[2], dDiam[2] / 4);
          ptCent[1] = Pnt_ProjPtTo(ptCent[1], ptCent[3], dDiam[2] / 4);

          ptCent[0] = Pnt_ProjPtTo(ptCent[0], ptCent[1], -dDiam[2] / 4);
          ptCent[1] = Pnt_ProjPtTo(ptCent[1], ptCent[0], -dDiam[2] / 4);
          pSeg->AddTail(new CPrimLine(1, 10, ptCent[0], ptCent[1]));
        }
        ::InvalidateRect(hwnd, nullptr, 1);
        dDiam[2] /= 2;
      ...
      case ID_OP6:  // Tee
      {
        ...
        if (pEndCapMark == nullptr)  // No end-cap consideration
        {
          if (wPrvKeyDwn == ID_OP2) {
            ...
            if (detsegs.SelSegAndPrimUsingPoint(ptCur) != nullptr)  // find diameter of duct
            {
            ...
            if (ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2]))  // Lead line vector is not nu
            {
              ...
              dDiam[1] = dDiam[2];
              dDiam[2] *= 2;
              rlpd::GenTransition(rlpd::End, &iJus[1], slope, &dDiam[1], &dDiam[2], &lnPar[2]);
              ptCur[0] = (lnPar[2][1][0] + lnPar[3][1][0]) * .5f;
              ptCur[1] = (lnPar[2][1][1] + lnPar[3][1][1]) * .5f;
              rlpd::bContSec = false;
              pgEndCapSeg = nullptr;
        ...
        pt[0] = Pnt_ProjPtTo(lnPar[3][0], lnPar[2][0], dDiam[2] / 4);
        pt[1] = Pnt_ProjPtTo(lnPar[3][1], ptCur, dDiam[2] * 0.05);
        pt[2] = lnPar[3][1];

        pt[3] = Pnt_ProjPtTo(lnPar[2][0], pt[0], dDiam[2] / 4);
        pt[4] = Pnt_ProjPtTo(lnPar[2][1], ptCur, dDiam[2] * 0.05);
        pt[5] = lnPar[2][1];
        ...
        pt[1] = Pnt_ProjPtTo(pt[2], pt[5], dDiam[2] * 0.25);
        pDoc->DoPrimDelete(pt[1]);
      ...
      case ID_OP8:
        ...
        if (rlpd::SelEndCapUsingPoint(pView, ptCur, pEndCapSeg, pEndCapMark)) {  // Endcap located
          ...
          ptPrv = pEndCapMark->GetPt();
          dDiam[1] = pEndCapMark->GetDat(0);  //duct width
          i = 0;
          //ptBeg = ptPrv + dDiam[1] / 4;		//find line on end cap mark
          ptBeg[2] = 0;
          while (detsegs.SelSegAndPrimUsingPoint(ptBeg) == nullptr && i < 500) {
            ptBeg = Pnt_RotAboutArbPtAndAx(ptBeg, ptPrv, vVwPlnNorm, 1);
            ptCur = ptBeg;
            i++;
          }
          if (detsegs.SelSegAndPrimUsingPoint(ptBeg) != nullptr) {
            if (detsegs.DetPrim()->Is(CPrim::Type::Line)) {  // endpoints of endcap line
              (static_cast<CPrimLine*>(detsegs.DetPrim()))->GetPts(ptBeg, ptEnd);
            }
          }
          ...
          pView->ModelViewTransform(ptView);
          if (detsegs.SelSegAndPrimAtCtrlPt(ptView) == nullptr) { break; }
          ...
          ptBeg = Pnt_ProjPtTo(ptUpDn[0], ptUpDn[1], dDiam[1] * 0.25);
          pDoc->DoPrimDelete(ptBeg);

          ptUpDn[2] = Pnt_ProjPtTo(ptUpDn[0], ptMid, rlpd::dInsideRadiusFactor * dDiam[1]);
          ptUpDn[3] = Pnt_ProjPtTo(ptUpDn[1], ptTemp, rlpd::dInsideRadiusFactor * dDiam[1]);
          ...
          if (LOWORD(wParam) == ID_OP8) { pstate.SetPen(pDC, nPenColor, nPenStyle); }
        }
        break;
      ...
      case ID_OP9: {
        ...
        ptBeg = ptCur;
        if (rlpd::SelEndCapUsingPoint(pView, ptBeg, pEndCapSeg, pEndCapMark)) {
          dDiam[1] = pEndCapMark->GetDat(0);
          rlpd::GenSizeNote(ptCur, dAng, dDiam[1]);  //if endcap found, get details and print dimension
          break;
        }
        ...
        if (detsegs.SelSegAndPrimUsingPoint(ptBeg) == nullptr) { break; }
        if (!detsegs.DetPrim()->Is(CPrim::Type::Line)) { break; }
        dAng = 0.;
        ...
        if (rlpd::SelEndCapUsingPoint(pView, ptMid, pEndCapSeg, pEndCapMark)) {
          dDiam[1] = pEndCapMark->GetDat(0);
          rlpd::GenSizeNote(ptCur, dAng, dDiam[1]);  //if endcap line, find details and print dimension
        } else                                       //find other line
        {
          ...
          (static_cast<CPrimLine*>(detsegs.DetPrim()))->GetPts(ptBeg, ptEnd);
          ptMid = Mid(ptBeg, ptEnd);
          if (rlpd::SelEndCapUsingPoint(pView, ptMid, pEndCapSeg,
                                        pEndCapMark))  //if endcap line, find details and print dimension
          {
            dDiam[1] = pEndCapMark->GetDat(0);
            rlpd::GenSizeNote(ptCur, dAng, dDiam[1]);
          }
        }
        break;
      }
...
void rlpd::SetOptions(double* section_diameter) {
  ...
  if (::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_DLGPROC_RLPD_OPTIONS), app.GetSafeHwnd(),
                  reinterpret_cast<DLGPROC>(DlgProcRLPDOptions)) > 0) {
    *section_diameter = std::max(0., rlpd::dSecDiam);
  }
}
...
bool rlpd::Fnd2LnsGivLn(CPrimLine* pTestLine, double dAccAng, CPrim*& pLeftLine, CLine* lnLeft, CPrim*& pRightLine,
                        CLine* lnRight) {
  CSeg* pSeg;
  CPrim* pPrim;
  POSITION posPrim;
  ...
  while (pos != nullptr) {
    ...
    while (posPrim != nullptr) {
      ...
      if (ptBeg == ptTestLineBeg || ptBeg == ptTestLineEnd) {  // Exchange points
        CPnt ptTmp = ptBeg;
        ptBeg = ptEnd;
        ptEnd = ptTmp;
      ...
      if (fabs(fabs(dLnAng0 - dLnAng) - HALF_PI) <= dAccAng) {
        if (pLeftLine == nullptr)  // No qualifiers yet
        {
          ...
          if (iRel1 ==
              CLine(ptTestLineBeg, ptTestLineEnd).DirRelOfPt(ptBeg))  // Both lines are on the same side of test line
          {
            pRightLine = pPrim;
            (*lnRight)(ptBeg, ptEnd);
...
int rlpd::GenElbow(double adWid, double adDep, CLine* pLns) {
  ...
  if (pgEndCapSeg != nullptr) {
    auto* pMark = static_cast<CPrimMark*>(pgEndCapSeg->GetHead());
  ...
  double dMinWid = std::min(dSecWid, adWid) * (rlpd::dInsideRadiusFactor - 0.5);
  double dMaxWid = std::max(dSecWid, adWid);
  ...
  if (pt[0] != ptIn[0]) pSeg->AddTail(new CPrimLine(pt[0], ptIn[0]));
  if (pt[2] != ptOut[0]) pSeg->AddTail(new CPrimLine(pt[2], ptOut[0]));
  if (iDir == -1) {
    ptIn[0] = ptIn[1];
    ptOut[0] = ptOut[1];
  }
  ...
  if (pt[1] != ptIn[1]) pSeg->AddTail(new CPrimLine(ptIn[1], pt[1]));
  if (pt[3] != ptOut[1]) pSeg->AddTail(new CPrimLine(ptOut[1], pt[3]));
...
void rlpd::GenSizeNote(CPnt arPt, double adAng,
                       double adDiam) {  // Generates text segment representing width and depth of a piece of duct.
...
bool rlpd::GenSection(double section_diameter, CLine* lines) {
  ...
  document->UpdateAllViews(nullptr, CPegDoc::HINT_SEG_SAFE, pSeg);
...
bool rlpd::SelEndCapUsingPoint(CPegView* pView, const CPnt& pt, CSeg*& pSeg, CPrimMark*& pMark) {
  CPnt ptProj;
...
double rlpd::TransitionLen(int aiJus,  // justification
                           //		0 centered
                           //		> 0 taper to right
                           //		< 0 taper to left
