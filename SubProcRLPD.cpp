...
LRESULT CALLBACK SubProcRLPD(HWND hwnd, UINT anMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (anMsg == WM_COMMAND)
	{
		...
		case ID_OP1:										// Search for an endcap in proximity of current location
			if (rlpd::SelEndCapUsingPoint(pView, ptCur, pEndCapSeg, pEndCapMark))
			{	// Endcap located
				char szLen[32];
				char* pszLen;
		...
		case ID_OP2:
			...
			{	// No end-cap consideration
				if (wPrvKeyDwn == ID_OP2)
				{
					...
					lnPar[1] = lnPar[3];
					lnLead(ptPrv, ptCur);
					if (ParLines_GenPts(.5, dDiam[2], lnLead, &lnPar[2]))
					{	// Lead line vector is not null
						if (rlpd::bContSec)
						{
							...
							else
							{
								...
								if (trapsegs.Remove(pgEndCapSeg) != 0)
								{
		...
		case ID_OP9:
		{
			...
			else				//find other line
			{
				...
				(static_cast<CPrimLine*>(detsegs.DetPrim()))->GetPts(ptBeg, ptEnd);
				ptMid = Mid(ptBeg, ptEnd);
				if (rlpd::SelEndCapUsingPoint(pView, ptMid, pEndCapSeg, pEndCapMark))	//if endcap line, find details and print dimension
				{
					dDiam[1] = pEndCapMark->GetDat(0);
