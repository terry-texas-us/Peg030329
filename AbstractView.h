...
class CAbstractView : public CObject {
 protected:
  ...
  double
      m_dNearClipDistance;  // distance from the target to the near (front) clipping plane along the target-camera line.
 ...
 public:  // Methods
  ...
  double* GetDirection(double* d) const {
    m_vDirection.Get(d);
    return (d);
  }
  ...
  double* GetTarget(double* d) const {
    m_ptTarget.Get(d);
    return (d);
  }
