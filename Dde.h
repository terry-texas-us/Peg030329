...
namespace dde
{
	...
	PEXECCMDFNINFO		ExecCmdAdd(const char* pszTopic, const char* pszCmdName, PEXECCMDFN pExecCmdFn, UINT uiMinArgs, UINT uiMaxArgs);
	PEXECCMDFNINFO		ExecCmdFind(PTOPICINFO pTopic, const char* lpszCmd);
	bool 				ExecCmdRemove(LPSTR pszTopic, LPSTR pszCmdName);

	PITEMINFO	 		ItemAdd(const char* lpszTopic, const char* lpszItem, LPWORD pFormatList, PREQUESTFN lpReqFn, PPOKEFN lpPokeFn);
