...
CTMat::CTMat(const CPnt& pt,     // reference point
...
CTMat::CTMat(const CPnt& ptO,   // origin of reference system
             ...
             const CVec& vYAx)  // y-axis of reference system
{
  ...
  *this *= tm.RotateZ(-vXAxT[1], vXAxT[0]);
  ...
  if (fabs(vYAxT[1]) <= DBL_EPSILON) return;
  ...
  if (fabs(vYAxT[0]) > DBL_EPSILON) {
    ...
    for (int i = 0; i < 4; i++) m_v[0][i] += m_v[1][i] * dShrFac;
  }
  Scale(vScale);
}
...
void CTMat::DefUsingArbPtAndAx(const CPnt& ptO,  // point on plane which defines origin
...
CPnt4 operator*(const CTMat& tm, const CPnt4& pt) {
  ...
  for (int i = 0; i < 4; i++) {
    ptOut[i] =
        pt.m_d[0] * tm.m_v[i][0] + pt.m_d[1] * tm.m_v[i][1] + pt.m_d[2] * tm.m_v[i][2] + pt.m_d[3] * tm.m_v[i][3];
  }
