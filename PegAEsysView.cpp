...
ON_WM_SIZE()
#pragma warning(pop)
ON_COMMAND(ID_3DVIEWS_TOP, &CPegView::On3dViewsTop)
ON_COMMAND(ID_3DVIEWS_BOTTOM, &CPegView::On3dViewsBottom)
ON_COMMAND(ID_3DVIEWS_LEFT, &CPegView::On3dViewsLeft)
ON_COMMAND(ID_3DVIEWS_RIGHT, &CPegView::On3dViewsRight)
ON_COMMAND(ID_3DVIEWS_FRONT, &CPegView::On3dViewsFront)
ON_COMMAND(ID_3DVIEWS_BACK, &CPegView::On3dViewsBack)
ON_COMMAND(ID_3DVIEWS_ISOMETRIC, &CPegView::On3dViewsIsometric)
ON_COMMAND(ID_3DVIEWS_AXONOMETRIC, &CPegView::On3dViewsAxonometric)
ON_COMMAND(ID_3DVIEWS_OBLIQUE, &CPegView::On3dViewsOblique)
ON_COMMAND(ID_3DVIEWS_PERSPECTIVE, &CPegView::On3dViewsPerspective)
ON_COMMAND(ID_BACKGROUNDIMAGE_LOAD, &CPegView::OnBackgroundImageLoad)
ON_COMMAND(ID_BACKGROUNDIMAGE_REMOVE, &CPegView::OnBackgroundImageRemove)
ON_COMMAND(ID_CAMERA_ROTATELEFT, &CPegView::OnCameraRotateLeft)
ON_COMMAND(ID_CAMERA_ROTATERIGHT, &CPegView::OnCameraRotateRight)
ON_COMMAND(ID_CAMERA_ROTATEUP, &CPegView::OnCameraRotateUp)
ON_COMMAND(ID_CAMERA_ROTATEDOWN, &CPegView::OnCameraRotateDown)
ON_COMMAND(ID_CURSOR_DEFAULT, &CPegView::OnCursorDefault)
ON_COMMAND(ID_CURSOR_SYMBOL, &CPegView::OnCursorSymbol)
ON_COMMAND(ID_CURSOR_CROSSHAIR, &CPegView::OnCursorCrosshair)
ON_COMMAND(ID_FILE_PLOT_QUARTER, &CPegView::OnFilePlotQuarter)
ON_COMMAND(ID_FILE_PLOT_HALF, &CPegView::OnFilePlotHalf)
ON_COMMAND(ID_FILE_PLOT_FULL, &CPegView::OnFilePlotFull)
// Standard printing commands
ON_COMMAND(ID_FILE_PRINT, &CPegView::OnFilePrint)
ON_COMMAND(ID_FILE_PRINT_DIRECT, OnFilePrint)
ON_COMMAND(ID_FILE_PRINT_PREVIEW, CView::OnFilePrintPreview)
ON_COMMAND(ID_HELP_KEY, &CPegView::OnHelpKey)
ON_COMMAND(ID_PRIM_PERPJUMP, &CPegView::OnPrimPerpJump)
ON_COMMAND(ID_PRIM_SNAPTO, &CPegView::OnPrimSnapTo)
ON_COMMAND(ID_RELATIVEMOVES_ENG_DOWN, &CPegView::OnRelativeMovesEngDown)
ON_COMMAND(ID_RELATIVEMOVES_ENG_DOWNROTATE, &CPegView::OnRelativeMovesEngDownRotate)
ON_COMMAND(ID_RELATIVEMOVES_ENG_IN, &CPegView::OnRelativeMovesEngIn)
ON_COMMAND(ID_RELATIVEMOVES_ENG_LEFT, &CPegView::OnRelativeMovesEngLeft)
ON_COMMAND(ID_RELATIVEMOVES_ENG_LEFTROTATE, &CPegView::OnRelativeMovesEngLeftRotate)
ON_COMMAND(ID_RELATIVEMOVES_ENG_OUT, &CPegView::OnRelativeMovesEngOut)
ON_COMMAND(ID_RELATIVEMOVES_ENG_RIGHT, &CPegView::OnRelativeMovesEngRight)
ON_COMMAND(ID_RELATIVEMOVES_ENG_RIGHTROTATE, &CPegView::OnRelativeMovesEngRightRotate)
ON_COMMAND(ID_RELATIVEMOVES_ENG_UP, &CPegView::OnRelativeMovesEngUp)
ON_COMMAND(ID_RELATIVEMOVES_ENG_UPROTATE, &CPegView::OnRelativeMovesEngUpRotate)
ON_COMMAND(ID_RELATIVEMOVES_DOWN, &CPegView::OnRelativeMovesDown)
ON_COMMAND(ID_RELATIVEMOVES_DOWNROTATE, &CPegView::OnRelativeMovesDownRotate)
ON_COMMAND(ID_RELATIVEMOVES_IN, &CPegView::OnRelativeMovesIn)
ON_COMMAND(ID_RELATIVEMOVES_LEFT, &CPegView::OnRelativeMovesLeft)
ON_COMMAND(ID_RELATIVEMOVES_LEFTROTATE, &CPegView::OnRelativeMovesLeftRotate)
ON_COMMAND(ID_RELATIVEMOVES_OUT, &CPegView::OnRelativeMovesOut)
ON_COMMAND(ID_RELATIVEMOVES_RIGHT, &CPegView::OnRelativeMovesRight)
ON_COMMAND(ID_RELATIVEMOVES_RIGHTROTATE, &CPegView::OnRelativeMovesRightRotate)
ON_COMMAND(ID_RELATIVEMOVES_UP, &CPegView::OnRelativeMovesUp)
ON_COMMAND(ID_RELATIVEMOVES_UPROTATE, &CPegView::OnRelativeMovesUpRotate)
ON_COMMAND(ID_SETUP_DIMLENGTH, &CPegView::OnSetupDimLength)
ON_COMMAND(ID_SETUP_DIMANGLE, &CPegView::OnSetupDimAngle)
ON_COMMAND(ID_SETUP_SCALE, &CPegView::OnSetupScale)
ON_COMMAND(ID_SETUP_UNITS, &CPegView::OnSetupUnits)
ON_COMMAND(ID_SETUP_CONSTRAINTS_AXIS, &CPegView::OnSetupConstraintsAxis)
ON_COMMAND(ID_SETUP_CONSTRAINTS_GRID, &CPegView::OnSetupConstraintsGrid)
ON_COMMAND(ID_SETUP_MOUSEBUTTONS, &CPegView::OnSetupMouseButtons)
ON_COMMAND(ID_VIEW_BACKGROUNDIMAGE, &CPegView::OnViewBackgroundImage)
ON_COMMAND(ID_VIEW_TRUETYPEFONTS, &CPegView::OnViewTrueTypeFonts)
ON_COMMAND(ID_VIEW_PENWIDTHS, &CPegView::OnViewPenWidths)
ON_COMMAND(ID_VIEW_ODOMETER, &CPegView::OnViewOdometer)
ON_COMMAND(ID_VIEW_REFRESH, &CPegView::OnViewRefresh)
ON_COMMAND(ID_VIEW_ZOOM, &CPegView::OnViewZoom)
ON_COMMAND(ID_VIEW_WINDOW, &CPegView::OnViewWindow)
ON_COMMAND(ID_VIEW_WIREFRAME, &CPegView::OnViewWireframe)
ON_COMMAND(ID_VIEW_RENDERED, &CPegView::OnViewRendered)
ON_COMMAND(ID_VIEW_PARAMETERS, &CPegView::OnViewParameters)
ON_COMMAND(ID_VIEW_SOLID, &CPegView::OnViewSolid)
ON_COMMAND(ID_VIEW_LIGHTING, &CPegView::OnViewLighting)
ON_COMMAND(ID_WINDOW_NORMAL, &CPegView::OnWindowNormal)
ON_COMMAND(ID_WINDOW_BEST, &CPegView::OnWindowBest)
ON_COMMAND(ID_WINDOW_LAST, &CPegView::OnWindowLast)
ON_COMMAND(ID_WINDOW_SHEET, &CPegView::OnWindowSheet)
ON_COMMAND(ID_WINDOW_ZOOMIN, &CPegView::OnWindowZoomIn)
ON_COMMAND(ID_WINDOW_ZOOMOUT, &CPegView::OnWindowZoomOut)
ON_COMMAND(ID_WINDOW_PAN, &CPegView::OnWindowPan)
ON_COMMAND(ID_WINDOW_PAN_LEFT, &CPegView::OnWindowPanLeft)
ON_COMMAND(ID_WINDOW_PAN_RIGHT, &CPegView::OnWindowPanRight)
ON_COMMAND(ID_WINDOW_PAN_UP, &CPegView::OnWindowPanUp)
ON_COMMAND(ID_WINDOW_PAN_DOWN, &CPegView::OnWindowPanDown)
#pragma warning(push)
#pragma warning(disable : 4191)
ON_UPDATE_COMMAND_UI(ID_VIEW_ODOMETER, &CPegView::OnUpdateViewOdometer)
ON_UPDATE_COMMAND_UI(ID_VIEW_TRUETYPEFONTS, &CPegView::OnUpdateViewTrueTypeFonts)
ON_UPDATE_COMMAND_UI(ID_VIEW_BACKGROUNDIMAGE, &CPegView::OnUpdateViewBackgroundImage)
ON_UPDATE_COMMAND_UI(ID_BACKGROUNDIMAGE_LOAD, &CPegView::OnUpdateBackgroundimageLoad)
ON_UPDATE_COMMAND_UI(ID_BACKGROUNDIMAGE_REMOVE, &CPegView::OnUpdateBackgroundimageRemove)
ON_UPDATE_COMMAND_UI(ID_VIEW_RENDERED, &CPegView::OnUpdateViewRendered)
ON_UPDATE_COMMAND_UI(ID_VIEW_WIREFRAME, &CPegView::OnUpdateViewWireframe)
ON_UPDATE_COMMAND_UI(ID_VIEW_PENWIDTHS, &CPegView::OnUpdateViewPenwidths)
#pragma warning(pop)
END_MESSAGE_MAP()

void CPegView::ModelViewPopActive() {
  if (!m_mvs.IsEmpty()) { m_mvActive = m_mvs.RemoveTail(); }
  m_mvActive.BuildTransformMatrix();
}

void CPegView::ModelViewPushActive() { m_mvs.AddTail(m_mvActive); }
...
void CPegView::ViewportPopActive() {
  if (!m_vps.IsEmpty()) { m_vpActive = m_vps.RemoveTail(); }
  opengl::Viewport(0, 0, int(m_vpActive.GetWidth()), int(m_vpActive.GetHeight()));
}
void CPegView::ViewportPushActive() { m_vps.AddTail(m_vpActive); }
...
void CPegView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) {
  ...
  if ((lHint & CPegDoc::HINT_SAFE) == CPegDoc::HINT_SAFE) { iPrimState = pstate.Save(); }
  if ((lHint & CPegDoc::HINT_ERASE) == CPegDoc::HINT_ERASE) { iDrawMode = pstate.SetROP2(pDC, R2_XORPEN); }
  if ((lHint & CPegDoc::HINT_TRAP) == CPegDoc::HINT_TRAP) { CPrim::SpecPenColor() = trapsegs.PenColor(); }
  switch (lHint) {
  ...
  if ((lHint & CPegDoc::HINT_TRAP) == CPegDoc::HINT_TRAP) { CPrim::SpecPenColor() = 0; }
  if ((lHint & CPegDoc::HINT_ERASE) == CPegDoc::HINT_ERASE) { pstate.SetROP2(pDC, iDrawMode); }
  if ((lHint & CPegDoc::HINT_SAFE) == CPegDoc::HINT_SAFE) { pstate.Restore(pDC, iPrimState); }
}
// CPegView diagnostics
#ifdef _DEBUG
void CPegView::AssertValid() const { CView::AssertValid(); }
void CPegView::Dump(CDumpContext& dc) const { CView::Dump(dc); }
...
void CPegView::OnLButtonDown(UINT, CPoint) { DoCustomMouseClick(leftMouseDown); }
...
void CPegView::OnRButtonDown(UINT, CPoint) { DoCustomMouseClick(rightMouseDown); }
void CPegView::OnRButtonUp(UINT, CPoint) { DoCustomMouseClick(rightMouseUp); }
void CPegView::OnMouseMove(UINT, CPoint point) {
  ...
  if (app.RubberBandingGetType() == Lines) {
    ...
    if (pnt[1].x != app.m_pntGinStart.x || pnt[1].y != app.m_pntGinStart.y) pDC->Polyline(pnt, 4);
    ...
    pDC->Ellipse(app.m_pntGinStart.x - iRadius, app.m_pntGinStart.y + iRadius, app.m_pntGinStart.x + iRadius,
                 app.m_pntGinStart.y - iRadius);
    app.m_pntGinEnd = point;
...
void CPegView::OnWindowNormal() { DoWindowPan(1.); }
void CPegView::OnWindowBest() {
  ...
  if (ptMin[0] < ptMax[0]) {
    ...
    CTMat tm;
    CPegDoc::GetDoc()->GetExtents(ptMin, ptMax, tm);
...
void CPegView::OnWindowZoomIn() { DoWindowPan(GetWidthInInches() / ModelViewGetUExt() * 1.25); }
void CPegView::OnWindowZoomOut() { DoWindowPan(GetWidthInInches() / ModelViewGetUExt() / 1.25); }
void CPegView::OnWindowPan() { DoWindowPan(GetWidthInInches() / ModelViewGetUExt()); }
void CPegView::OnWindowPanLeft() { DoWindowPan0(ID_WINDOW_PAN_LEFT); }
void CPegView::OnWindowPanRight() { DoWindowPan0(ID_WINDOW_PAN_RIGHT); }
void CPegView::OnWindowPanUp() { DoWindowPan0(ID_WINDOW_PAN_UP); }
void CPegView::OnWindowPanDown() { DoWindowPan0(ID_WINDOW_PAN_DOWN); }
void CPegView::OnCameraRotateLeft() { DoCameraRotate(ID_CAMERA_ROTATELEFT); }
void CPegView::OnCameraRotateRight() { DoCameraRotate(ID_CAMERA_ROTATERIGHT); }
void CPegView::OnCameraRotateUp() { DoCameraRotate(ID_CAMERA_ROTATEUP); }
void CPegView::OnCameraRotateDown() { DoCameraRotate(ID_CAMERA_ROTATEDOWN); }
...
void CPegView::OnSetupConstraintsAxis() {
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SETUP_CONSTRAINTS_AXIS), GetSafeHwnd(),
              DlgProcSetupConstraintsAxis);
  app.StatusLineDisplay();
}
...
void CPegView::DoCustomMouseClick(const std::string& customCommand) const {
  ...
  while (pos < customCommand.size()) {
    if (customCommand[pos] == '{') {
      ...
      while (pos < customCommand.size() && std::isdigit(static_cast<unsigned char>(customCommand[pos]))) { ++pos; }
...
void CPegView::OnUpdateViewWireframe(CCmdUI* pCmdUI) { pCmdUI->SetCheck(m_bViewWireframe); }

void CPegView::OnUpdateViewPenwidths(CCmdUI* pCmdUI) { pCmdUI->SetCheck(m_bViewPenWidths); }
