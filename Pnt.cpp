...
bool operator!=(const CPnt& ptA, const CPnt& ptB) { return !(ptA == ptB); }
...
CPnt Mid(const CPnt& ptA, const CPnt& ptB) { return ptA + .5 * (ptB - ptA); }
...
bool p2DPtInWnd(const CPnt& pt, CPnt* ptWnd) {
  ...
  if (ptWnd[0][0] > pt[0] + dReps || ptWnd[1][0] < pt[0] - dReps) return false;

  dReps = DBL_EPSILON + fabs(DBL_EPSILON * pt[1]);
  if (ptWnd[0][1] > pt[1] + dReps || ptWnd[1][1] < pt[1] - dReps) return false;
