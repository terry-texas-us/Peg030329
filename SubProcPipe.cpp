...
namespace pipe {
double dTicSize{0.03125};
double pipe::dCSize{0.03125};
WORD pipe::wCurSymId{0};
}  // namespace pipe
LRESULT CALLBACK SubProcPipe(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam) noexcept {
  static CPnt pt[6]{};

  static WORD wPrvKeyDwn{0};

  static CPrimArc* pArcVer{nullptr};
  static CPrimLine* pLineHor{nullptr};
  ...
  if (nMsg == WM_COMMAND) {
    ...
    switch (LOWORD(wParam)) {
      case ID_OP0:
        ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SUBPROC_PIPE_OPTIONS), app.GetSafeHwnd(),
                    reinterpret_cast<DLGPROC>(DlgProcPipeOptions));
        break;
        ...
        while (eJoinHor == pipe::No && eJoinVer == pipe::No && pos != nullptr) {
          ...
          while (eJoinHor == pipe::No && eJoinVer == pipe::No && posPrim != nullptr) {
            ...
            if (pPrim->Is(CPrim::Type::Arc)) {  // Primitive is potentially an existing vertical pipe section
              ...
              if (pLine->SelUsingPoint(pView, ptView, .02, ptProj)) {
                pLineHor = pLine;
                eJoinHor = (wPrvKeyDwn == 0) ? pipe::Begin : pipe::End;
                pt[1] = ptProj;
                pt[1] = pView->ModelViewGetMatrixInverse() * pt[1];
              }
            }
          }
        }
      ...
      case ID_OP3:
        if (eJoinHor != pipe::No) {  // On an existing horizontal pipe section
      ...
      case ID_OP4:
        ...
        if (eJoinHor != pipe::No) {  // On an existing horizontal pipe section
          ...
          if (eJoinHor == pipe::End) {  // Dropping into an existing horizontal pipe section
          ...
          } else {  // Dropping from an existing horizontal pipe section
            pLineHor->SetPt1(pt[1]);
      ...
      case ID_OP5:
        if (eJoinHor != pipe::No) {  // On an existing horizontal pipe section
          ...
          if (eJoinHor == pipe::End) {  // Rising into an existing horizontal pipe section
            pLineHor->SetPt1(pt[1]);
            pipe::GenTicMark(pt[1], ptBeg, 2. * pipe::dCSize);
            ...
            pstate.SetPen(pDC, nPenColor, nPenStyle);
            pipe::GenFits(wPrvKeyDwn, pt[0], ID_OP5, pt[1]);
          } else {  // Rising from an existing horizontal pipe section
            pt[3] = Pnt_ProjPtTo(pt[1], ptBeg, pipe::dCSize);
            pt[4] = Pnt_ProjPtTo(pt[1], ptEnd, pipe::dCSize);
      ...
      case ID_OP9:                    // Generate wye
        if (eJoinHor == pipe::End) {  // Save location of point on existing horizontal pipe
          pt[2] = pt[1];
...
void pipe::GenSyms(CDC* pDC, const CPnt& P0) {
  double dSymSiz[] = {.09375, .09375, .09375, .09375, .125, .125, .125, .125, .125,
                      .125,   .125,   .125,   .125,   .125, .125, 0.,   0.,   .09375};
  double dTicDis[] = {.125,   .125,   .125,   .125,   .15625, .15625, .15625, .15625, .15625,
  ...
  while (pos != nullptr) {
    ...
    while (posPrim != nullptr) {
      ...
      if (pPrim->Is(CPrim::Type::Line)) {  // Primitive is potentially an existing horizontal pipe section
        ...
        if (pLine->SelUsingPoint(pView, ptView, .02, ptIns)) {
          ptIns = pView->ModelViewGetMatrixInverse() * ptIns;
...
FoundIt:
  ...
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SUBPROC_PIPE_SYMBOL), app.GetSafeHwnd(),
              reinterpret_cast<DLGPROC>(DlgProcPipeSymbol));
...
void pipe::GenTicMark(const CPnt& ptBeg, const CPnt& ptEnd, double dDis) {
  ...
  if (dProjEnd > DBL_EPSILON) {  // Tic does not intersect line at end point
