...
bool text_Display(CPegView* pView, CDC* pDC, const CFontDef& fd, const CRefSys& rs, const CString& strText) {
  ...
  if (nSiz == 0) return true;

  CPnt4 ptPvt(rs.Origin(), 1.);
  CVec4 vXAxRef(rs.DirX(), 0.);
  CVec4 vYAxRef(rs.DirY(), 0.);

  pView->ModelViewTransform(ptPvt);
  pView->ModelViewTransform(vXAxRef);
  pView->ModelViewTransform(vYAxRef);

  CVec vPlnNorm = CVec(vXAxRef) ^ CVec(vYAxRef);
  vPlnNorm.Normalize();

  if (vPlnNorm != ZDIR) return false;
