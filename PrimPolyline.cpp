...
CPrimPolyline::CPrimPolyline() { m_wFlags = 0; }
...
CPrimPolyline::CPrimPolyline(size_t numberOfPoints, std::vector<CPnt>& points) {
  m_nPenColor = pstate.PenColor();
  m_nPenStyle = pstate.PenStyle();
...
CPrimPolyline::CPrimPolyline(const CPrimPolyline& other) {
  ...
  m_nPenStyle = other.m_nPenStyle;
  m_wFlags = other.m_wFlags;
  m_pts.Copy(other.m_pts);
}

const CPrimPolyline& CPrimPolyline::operator=(const CPrimPolyline& other) {
  m_nPenColor = other.m_nPenColor;
  m_nPenStyle = other.m_nPenStyle;
  m_wFlags = other.m_wFlags;
  m_pts.Copy(other.m_pts);
...
CPnt CPrimPolyline::GoToNxtCtrlPt() const {
  ...
  if (mS_wPivotVertex >= wPts) {  // have not yet rocked to a vertex
    WORD wBeg = WORD(mS_wEdge - 1);
    WORD wEnd = WORD(mS_wEdge % wPts);
...
bool CPrimPolyline::SelUsingPoint(CPegView* pView, const CPnt4& pt, double dTol, CPnt& ptProj) {
  ...
  WORD wPts = WORD(m_pts.GetSize());
  if (mS_wEdgeToEvaluate > 0 && mS_wEdgeToEvaluate <= wPts) {  // Evaluate specified edge of polyline
