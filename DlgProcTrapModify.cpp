...
void DlgProcTrapModifyDoOK(HWND hDlg) {
  if (::SendDlgItemMessage(hDlg, IDC_MOD_PEN, BM_GETCHECK, 0, 0L)) trapsegs.ModifyPenColor(pstate.PenColor());
  if (::SendDlgItemMessage(hDlg, IDC_MOD_LINE, BM_GETCHECK, 0, 0L)) trapsegs.ModifyPenStyle(pstate.PenStyle());
  if (::SendDlgItemMessage(hDlg, IDC_MOD_FILL, BM_GETCHECK, 0, 0L)) DlgProcTrapModifyPolygons();
