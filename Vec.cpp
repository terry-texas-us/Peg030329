...
CVec::CVec(const CLine& ln) { *this = ln[1] - ln[0]; }
...
CVec CVec::operator^(const CVec& v) const {  // Determines the cross product of two vectors.
...
bool CVec::Identical(const CVec& v, double dTol) const {
  return (fabs(m_d[0] - v.m_d[0]) > dTol || fabs(m_d[1] - v.m_d[1]) > dTol || fabs(m_d[2] - v.m_d[2]) > dTol ? false
                                                                                                             : true);
}

bool CVec::IsNull(double dTol) const {
  return (fabs(m_d[0]) > dTol || fabs(m_d[1]) > dTol || fabs(m_d[2]) > dTol ? false : true);
}
...
bool CVec::Collinear(const CVec& v, double dTol) const {
  if (fabs(m_d[0]) > FLT_EPSILON) {
    ...
    return (fabs(dT * m_d[1] - v.m_d[1]) > dTol || fabs(dT * m_d[2] - v.m_d[2]) > dTol) ? false : true;
  } else if (fabs(m_d[1]) > FLT_EPSILON) {
    double dT = v.m_d[1] / m_d[1];
    return (fabs(dT * m_d[0] - v.m_d[0]) > dTol || fabs(dT * m_d[2] - v.m_d[2]) > dTol) ? false : true;
...
void CVec::RotAboutArbAx(const CVec& vRefAx,  // unit vector defining rotation axis
...
double Vec_Angle_xy(const CVec& v) {
  ...
  if (fabs(v.m_d[0]) > DBL_EPSILON || fabs(v.m_d[1]) > DBL_EPSILON) {
    ...
    if (dRetVal < 0.) dRetVal += TWOPI;
  }
  return (dRetVal);
}

double Vec_Length_xy(const CVec& v) { return (sqrt(v.m_d[0] * v.m_d[0] + v.m_d[1] * v.m_d[1])); }
