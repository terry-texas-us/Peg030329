...
LRESULT CALLBACK SubProcPower(HWND hwnd, UINT nMsg, WPARAM wParam, LPARAM lParam) noexcept {
  ...
  static CPrimLine* pLinePrv{nullptr};
  static CSeg* pSegPrv{nullptr};
  ...
  if (nMsg == WM_COMMAND) {
    ...
    switch (LOWORD(wParam)) {
      ...
      case ID_OP1: {  // Looking for electrical symbol (actually any circle)
        bArrow = false;
        bConductor = false;
      ...
      case ID_OP2:
        ...
        if (wPrvKeyDwn == ID_OP2) {
          ...
          if (dDis > DBL_EPSILON && dDis / std::max(DBL_EPSILON, Pnt_DistanceTo_xy(pt[0], pt[1])) <= 0.258819) {
            pLinePrv->SetPt1(pt2);
            pt[0] = pt2;
          }
        } else if (wPrvKeyDwn == ID_OP3) {  // Circuit from a symbol
        ...
        } else {               // Circuit to or from a symbol
          if (eJoin == End) {  // Ending circuit at symbol
      ...
      case ID_OP7:
        ...
        if (!bConductor || Pnt_DistanceTo_xy(pt1, pt[1]) > .05) {
          ...
          // error
          pt[0] = pt[1];
        } else if (Pnt_DistanceTo_xy(pt2, pt3) > .04) {  // Room for additional conductors on identified line
        l400:
          switch (LOWORD(wParam)) {
            case ID_OP4:
        ...
        } else {  // No more room for additional conductors on identified line
          bConductor = false;
          // error
          pt[0] = pt3;
        }
      ...
      case ID_OP8:
        ...
        if (!bArrow || Pnt_DistanceTo_xy(pt1, pt[1]) > .05) {
          ...
          if (SubProcPowerFndCirc(ln, pt[1], pt1, &dRel)) {
            if (dRel <= .5) {
              pt3 = ln[1];
              if (Pnt_DistanceTo_xy(pt1, ln[0]) <= .1) pt1 = ln[0];
            } else {
              pt3 = ln[0];
              if (Pnt_DistanceTo_xy(pt1, ln[1]) <= .1) pt1 = ln[1];
            }
            pt2 = pt1;
            bArrow = true;
            goto l300;
          ...
          // error
          pt[0] = pt[1];
        } else if (Pnt_DistanceTo_xy(pt2, pt3) >
                   .04) {  // Room for additional circuit run to home arrows on identified line
        l300:
          pt[2] = Pnt_ProjPtTo(pt2, pt3, .05);
  ...
  while (pos != nullptr) {
    ...
    while (posPrim != nullptr) {
      ...
      if (pPrim->Is(CPrim::Type::Line)) {
        ...
        if (line::EvalPtProx_xy(ln, pt, .05, ptProj, dRel, &dDis)) return (1);
      }
    }
  }
  return 0;
