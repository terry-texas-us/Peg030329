...
void dde::Uninitialize() {
  ...
  if (ServerInfo.pfnCustomCallback) { ServerInfo.pfnCustomCallback = nullptr; }
  ...
  while (pData && *pData) {  // Parse and execute each command in turn
    ...
    if (!bResult) {
      if (pCI && pCI->pResultItem)
        ...
        pCI->pResultItem->hData = DdeCreateDataHandle(ServerInfo.dwInstance, (LPBYTE)szResult, lstrlen(szResult) + 1u,
                                                      0, pCI->pResultItem->hszItemName, CF_TEXT, 0);
...
bool dde::ParseCmd(LPSTR* ppszCmdLine, PTOPICINFO pTopic, LPSTR pszError, UINT uiErrorSize, PPOP pOpTable, UINT,
                   LPSTR pArgBuf) {
  LPSTR pArg;
  PPOP ppOp = pOpTable;
