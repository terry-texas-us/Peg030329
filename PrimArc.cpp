...
///<summary>Arc is constructed using a center point, a major and minor vector and a sweep ang.</summary>
CPrimArc::CPrimArc(PENCOLOR nPenColor, PENSTYLE nPenStyle, const CPnt& ptCP, const CVec& vMajAx, const CVec& vMinAx,
                   double dSwpAng)
    : m_ptCenter(ptCP), m_vMajAx(vMajAx), m_vMinAx(vMinAx) {
...
void CPrimArc::GetXYExtents(CPnt arBeg, CPnt arEnd, CPnt* arMin,
                            CPnt* arMax) {  // Determines the xy extent of an arc segment.
...
bool CPrimArc::IsPtACtrlPt(CPegView* pView, const CPnt4& ptPic) const {
  ...
  for (WORD w = 0; w < 2; w++) {
    ...
    if (Pnt4_DistanceTo_xy(ptPic, pt[w]) < mS_dPicApertSiz) return true;
  }
  return false;
}
...
int CPrimArc::IsWithinArea(const CPnt& ptLL, const CPnt& ptUR, CPnt* ptInt) {
  ...
  if (ptMin[0] >= ptLL[0] && ptMax[0] <= ptUR[0] && ptMin[1] >= ptLL[1] &&
      ptMax[1] <= ptUR[1]) {  // Totally within window boundaries
    ptInt[0] = ptBeg;
    ptInt[1] = ptEnd;
    return (2);
  }
  ...
  if (ptMax[0] > ptUR[0]) {  // Arc may intersect with right window boundary
  ...
  if (ptMax[1] > ptUR[1]) {  // Arc may intersect with top window boundary
  ...
  if (ptMin[0] < ptLL[0]) {  // Arc may intersect with left window boundary
  ...
  if (ptMin[1] < ptLL[1]) {  // Arc may intersect with bottom window boundary
  ...
  double dBegAng =
      atan2((double)(ptBeg[1] - m_ptCenter[1]), (double)(ptBeg[0] - m_ptCenter[0]));  // Arc begin angle (- pi to pi)

  double dIntAng[8]{};
  double dWrkAng;
  int iInts = 0;
  for (int i2 = 0; i2 < iSecs; i2++) {  // Loop thru possible intersections
    dWrkAng = atan2((double)(ptWrk[i2][1] - m_ptCenter[1]),
                    (double)(ptWrk[i2][0] - m_ptCenter[0]));  // Current intersection angle (- pi to
    dIntAng[iInts] = dWrkAng - dBegAng;                       // Sweep from begin to intersection
    if (dIntAng[iInts] < 0.) dIntAng[iInts] += TWOPI;
    if (fabs(dIntAng[iInts]) - m_dSwpAng < 0.) {  // Intersection lies on arc
      int i;
  ...
  //else
  {
    if (ptBeg[0] >= ptLL[0] && ptBeg[0] <= ptUR[0] && ptBeg[1] >= ptLL[1] &&
        ptBeg[1] <= ptUR[1]) {  // Add beg point to int set
      for (int i = iInts; i > 0; i--) ptInt[i] = ptInt[i - 1];
      ptInt[0] = ptBeg;
      iInts++;
    }
    if (ptEnd[0] >= ptLL[0] && ptEnd[0] <= ptUR[0] && ptEnd[1] >= ptLL[1] &&
        ptEnd[1] <= ptUR[1]) {  // Add end point to int set
      ptInt[iInts] = ptEnd;
      iInts++;
    }
  }
...
bool CPrimArc::IsInView(CPegView* pView) const {
  CPnts pts;
  ...
  for (WORD w = 1; w < 4; w++) {
    ...
    if (Pnt4_ClipLine(ptBeg, ptEnd)) return true;
...
bool CPrimArc::SelUsingPoint(CPegView* pView, const CPnt4& ptPic, double dTol, CPnt& ptProj) {
  polyline::BeginLineStrip();
  GenPts(m_ptCenter, m_vMajAx, m_vMinAx, m_dSwpAng);
  return (polyline::SelUsingPoint(pView, ptPic, dTol, mS_dRel, ptProj));
}
bool CPrimArc::SelUsingRect(CPegView* pView, const CPnt& pt1, const CPnt& pt2) {
  polyline::BeginLineStrip();
  GenPts(m_ptCenter, m_vMajAx, m_vMinAx, m_dSwpAng);
...
double CPrimArc::SwpAngToPt(const CPnt& pt) {
  ...
  return (line::AngleBetweenLn_xy(CLine(ORIGIN, ptBeg), CLine(ORIGIN, ptEnd)));
}
...
bool CPrimArc::Write(CFile& fl) const {
  ...
  m_vMajAx.Write(fl);
  m_vMinAx.Write(fl);
  FilePeg_WriteDouble(fl, m_dSwpAng);
