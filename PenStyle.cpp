...
CPenStyle::CPenStyle(const CString& strName, const CString& strDesc, WORD wLens, double* dLen) {
  ...
  if (m_wLens == 0)
    ...
    for (int i = 0; i < m_wLens; i++) { m_dLen[i] = std::max(-99., std::min(dLen[i], 99.)); }
  }
}
CPenStyle::CPenStyle(const CPenStyle& penstyle) {
  ...
  m_dLen = new double[m_wLens];
  for (int i = 0; i < m_wLens; i++) { m_dLen[i] = penstyle.m_dLen[i]; }
}
CPenStyle::~CPenStyle() { delete[] m_dLen; }
...
double CPenStyle::GetPatternLen() const {
  ...
  for (int i = 0; i < m_wLens; i++) dLen += fabs(m_dLen[i]);
