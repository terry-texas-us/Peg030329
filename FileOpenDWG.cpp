  ...
  for (long lBlockHeader = 0; lBlockHeader < lNumBlockHeaders - 2; lBlockHeader++) {
    if (adGetBlockheader(hdb, &m_htb->blkh) == 1) {
      if (!m_htb->blkh.purgedflag) {
        if (adStartEntityGet(m_htb->blkh.entitylist) == 0) continue;
        if (adGetEntity(m_htb->blkh.entitylist, m_henhd, m_hen) == 0) continue;
        if (m_henhd->enttype != AD_ENT_BLOCK) continue;
  ...
  for (short entset = 0; entset < 2; entset++) {
    ...
    do {
      ...
      if (nSuccess) {
        ...
        if (pPrim == 0) {  // entity not loaded
          if (m_henhd->enttype == AD_ENT_ATTRIB) {
            iAttribEnts++;
          ...
          if (m_dhd->tilemode == 0 && entset == 1) {
            for (int i = 1; i < m_iVpCount; i++) {
              ...
              pView->ModelViewSetMatrix(m_tmView[i]);
  ...
  if (m_dhd->tilemode == 1) {
    ...
    while (adGetEntity(blkh.entitylist, m_henhd, m_hen)) {
      if (m_henhd->enttype == AD_ENT_VIEWPORT) {
        ...
        pView->ModelViewPushActive();
        pView->ModelViewLookAt(m_av[m_iVpCount].GetEye(), m_av[m_iVpCount].GetTarget(), m_av[m_iVpCount].GetVwUp());
        pView->ModelViewSetNearClipDistance(m_av[m_iVpCount].GetNearClipDistance());
        pView->ModelViewSetFarClipDistance(m_av[m_iVpCount].GetFarClipDistance());
        pView->ModelViewSetWnd(m_av[m_iVpCount].GetUMin(), m_av[m_iVpCount].GetVMin(), m_av[m_iVpCount].GetUMax(),
                               m_av[m_iVpCount].GetVMax());
        m_tmView[m_iVpCount] = pView->ModelViewGetMatrix();
        pView->ModelViewPopActive();
...
void CFileOpenDWG::ReadLayerTable() {
  ...
  for (int i = 0; i < int(adNumLayers(m_hdb)); i++) {
    ...
    if (CPegDoc::GetDoc()->LayersLookup(m_htb->lay.name) < 0) {
      ...
      if (m_htb->lay.color < 0 || (m_htb->lay.flag & AD_LAYER_FROZEN) != 0) pLayer->SetStateOff();
...
void CFileOpenDWG::ReadLnTypsTable() {
  ...
  for (int i = 0; i < int(adNumLinetypes(m_hdb)); i++) {
    ...
    if (m_htb->ltype.purgedflag) continue;

    if (CPegDoc::GetDoc()->PenStylesLookup(m_htb->ltype.name) != PENSTYLE_LookupErr) continue;
    ...
    for (short n = 0; n < nDashLens; n++) dDashLen[n] = m_htb->ltype.segdata[n].val;
...
CPrimArc::CPrimArc(PAD_ENT_HDR henhd, PAD_ENT hen) {
  ...
  if (henhd->enttype == AD_ENT_CIRCLE) {
    ...
    if (dStAng >= TWOPI) {  // need to rationalize angs to first period angles in range on (0 to twopi)
      dStAng -= TWOPI;
      dEndAng -= TWOPI;
    }
    ...
    if (dAng <= FLT_EPSILON) dAng += TWOPI;
...
#if ODA_FUNCTIONALITY
bool CPrimBSpline::Write(AD_DB_HANDLE, AD_VMADDR, PAD_ENT_HDR, PAD_ENT) { return false; }
#endif

#if ODA_FUNCTIONALITY
bool CPrimDim::Write(AD_DB_HANDLE, AD_VMADDR, PAD_ENT_HDR, PAD_ENT) { return false; }
#endif
...
CPrimPolygon::CPrimPolygon(AD_DB_HANDLE hdb, PAD_ENT_HDR henhd, PAD_ENT hen) {
  if (henhd->enttype == AD_ENT_FACE3D) {
    ...
    m_wPts = WORD((hen->solid.pt2[0] == hen->solid.pt3[0] && hen->solid.pt2[1] == hen->solid.pt3[1] &&
                   hen->solid.pt2[2] == hen->solid.pt3[2])
                      ? 3
                      : 4);
    ...
    if (m_wPts == 4) { m_Pt[3] = CPnt(hen->solid.pt2[0], hen->solid.pt2[1], hen->solid.pt2[2]); }
    m_ptOrig = m_Pt[0];
    m_vPosXAx = XDIR;
    m_vPosYAx = YDIR;
    ...
    if (hen->hatch.boundaryobjblob != AD_VMNULL) { bcptrAssocObj = adStartBlobRead(hen->hatch.boundaryobjblob); }
    ...
    if (hen->hatch.boundaryobjblob != AD_VMNULL) { adEndBlobRead(bcptrAssocObj); }
  }
}
#endif
...
CPrimPolyline::CPrimPolyline(PAD_ENT hen) {
  ...
  for (WORD w = 0; w < wPts; w++) {
    ...
    if (hen->lwpline.flag & AD_LWPLINE_HAS_BULGES) adReadBlobDouble(bcptr, &dbulge);
    if (hen->lwpline.flag & AD_LWPLINE_HAS_WIDTHS) adReadBlob2Double(bcptr, dwidth);
  }
  adEndBlobRead(bcptr);
}
#endif
...
bool CPrimPolyline::Write(AD_DB_HANDLE hdb, AD_VMADDR entlist, PAD_ENT_HDR henhd, PAD_ENT hen) {
  ...
  for (WORD w = 0; w < m_pts.GetSize(); w++) {
    ...
    if (m_wFlags & AD_LWPLINE_HAS_BULGES) adWriteBlobDouble(bcptr, &dBulge);
    if (m_wFlags & AD_LWPLINE_HAS_WIDTHS) adWriteBlob2Double(bcptr, dWidth);
  }
  adEndBlobWrite(bcptr);
  adAddEntityToList(hdb, entlist, henhd, hen);
...
#if ODA_FUNCTIONALITY
bool CPrimTag::Write(AD_DB_HANDLE, AD_VMADDR, PAD_ENT_HDR, PAD_ENT) { return false; }
#endif
...
CPrimText::CPrimText(AD_DB_HANDLE hdb, PAD_ENT_HDR henhd, PAD_ENT hen) {
  ...
  if (henhd->enttype == AD_ENT_TEXT) {
    /*switch (hen->text.tdata.vertalignment)
        ...
        */
    m_fd.Set(CFontDef::PREC_TRUETYPEFONT, strFont, CFontDef::PATH_RIGHT, CFontDef::HOR_ALIGN_LEFT,
             CFontDef::VER_ALIGN_BOTTOM, 0.);
    CCharCellDef ccd(hen->text.tdata.rotang, hen->text.tdata.oblique, hen->text.tdata.widthfactor,
                     hen->text.tdata.height);
    ...
    if (hen->mtext.ldblob == AD_VMNULL)
      ...
      while (adReadMtextBlock(bcptr, hen->mtext.textstr)) m_strText += hen->mtext.textstr;
      adEndBlobRead(bcptr);
    }
    int n = 0;
