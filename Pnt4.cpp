...
CPnt4::CPnt4(const CPnt& pt, const double d) {
  m_d[0] = pt[0];
  m_d[1] = pt[1];
  m_d[2] = pt[2];
  m_d[3] = d;
...
CPnt4 CPnt4::operator*(const double t) const { return CPnt4(m_d[0] * t, m_d[1] * t, m_d[2] * t, m_d[3] * t); }
CPnt4 CPnt4::operator/(const double t) const { return CPnt4(m_d[0] / t, m_d[1] / t, m_d[2] / t, m_d[3] / t); }
...
CPnt4 operator*(const double d, const CPnt4& pt) { return pt * d; }
bool operator==(const CPnt4& ptA, const CPnt4& ptB) {
  return (ptA.m_d[0] == ptB.m_d[0]) && (ptA.m_d[1] == ptB.m_d[1]) && (ptA.m_d[2] == ptB.m_d[2]) &&
         (ptA.m_d[3] == ptB.m_d[3]);
}
bool operator!=(const CPnt4& ptA, const CPnt4& ptB) { return !(ptA == ptB); }
...
CPnt4 Max(const CPnt4& ptA, const CPnt4& ptB) {
  return CPnt4(std::max(ptA.m_d[0], ptB.m_d[0]), std::max(ptA.m_d[1], ptB.m_d[1]), std::max(ptA.m_d[2], ptB.m_d[2]),
               std::max(ptA.m_d[3], ptB.m_d[3]));
}
CPnt4 Min(const CPnt4& ptA, const CPnt4& ptB) {
  return CPnt4(std::min(ptA.m_d[0], ptB.m_d[0]), std::min(ptA.m_d[1], ptB.m_d[1]), std::min(ptA.m_d[2], ptB.m_d[2]),
               std::min(ptA.m_d[3], ptB.m_d[3]));
}
bool Pnt4_ClipLine(CPnt4& ptA, CPnt4& ptB) {
  double dBoundaryCodeA[] = {ptA.m_d[3] + ptA.m_d[0], ptA.m_d[3] - ptA.m_d[0], ptA.m_d[3] + ptA.m_d[1],
                             ptA.m_d[3] - ptA.m_d[1], ptA.m_d[3] + ptA.m_d[2], ptA.m_d[3] - ptA.m_d[2]};
  double dBoundaryCodeB[] = {ptB.m_d[3] + ptB.m_d[0], ptB.m_d[3] - ptB.m_d[0], ptB.m_d[3] + ptB.m_d[1],
                             ptB.m_d[3] - ptB.m_d[1], ptB.m_d[3] + ptB.m_d[2], ptB.m_d[3] - ptB.m_d[2]};

  int iOutCodeA = 0;
  int iOutCodeB = 0;

  for (int iBC = 0; iBC < 6; iBC++) {
    if (dBoundaryCodeA[iBC] <= 0.) iOutCodeA |= (1 << iBC);
    if (dBoundaryCodeB[iBC] <= 0.) iOutCodeB |= (1 << iBC);
  }

  if ((iOutCodeA & iOutCodeB) != 0) return false;
  if ((iOutCodeA | iOutCodeB) == 0) return true;
  ...
  if (iOutCodeA != 0) { ptA = pt + dTIn * (ptB - pt); }
  if (iOutCodeB != 0) { ptB = pt + dTOut * (ptB - pt); }
  return true;
}
