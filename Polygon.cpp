...
void Polygon_Display(CPegView* pView, CDC* pDC, CPnt4s& pta) {
  int iPts = (int)pta.GetSize();
  if (iPts >= 2) {
    CPoint* pnt = new CPoint[static_cast<size_t>(iPts)];

    pView->DoProjection(pnt, pta);
...
void Polygon_GenPts(const CPnt& ptCP, const CVec& vXRefAx, const CVec& vYRefAx, int iPts, CPnt* pt) {
  ...
  for (int i = 0; i < iPts; i++) { pt[i] = tm * pt[i]; }
}
...
void Polygon_IntersectionWithPln(CPnt4s& ptaIn, const CPnt4& ptQ, const CVec4& vQNorm, CPnt4s& ptaOut) {
  ...
  if (bVisVer0) { ptaOut.Add(ptaIn[0]); }
  ...
  for (int i = 1; i < iPtsIn; i++) {
    ...
    if (bEdgeVis[1]) { ptaOut.Add(ptaIn[i]); }
    ptEdge[0] = ptEdge[1];
    bEdgeVis[0] = bEdgeVis[1];
  }
  if (ptaOut.GetSize() != 0 && bEdgeVis[0] != bVisVer0) {  // first and last vertices on opposite sides of clip plane
...
void Pnt4_ClipPolygon(CPnt4s& pta) {
  static CPnt4 ptPln[] = {CPnt4(-1., 0., 0., 1.), CPnt4(1., 0., 0., 1.),  CPnt4(0., -1., 0., 1.),
                          CPnt4(0., 1., 0., 1.),  CPnt4(0., 0., -1., 1.), CPnt4(0., 0., 1., 1.)};

  static CVec4 vPln[] = {CVec4(1., 0., 0., 0.),  CVec4(-1., 0., 0., 0.), CVec4(0., 1., 0., 0.),
                         CVec4(0., -1., 0., 0.), CVec4(0., 0., 1., 0.),  CVec4(0., 0., -1., 0.)};
