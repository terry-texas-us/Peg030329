...
CTMat CPrimSegRef::BuildTransformMatrix(const CPnt& ptBase) const {
  CTMat tm1;
  tm1.Translate(ORIGIN - ptBase);
  CTMat tm2;
  tm2.Scale(m_vScale);
  CTMat tm3;
  tm3.RotateZ(sin(m_dRotation), cos(m_dRotation));
  CTMat tm4(ORIGIN, m_vZ);
  CTMat tm5;
  tm5.Translate(m_pt - ORIGIN);
...
void CPrimSegRef::Transform(const CTMat& tm) {
  ...
  if (fabs(m_vZ[0]) <= FLT_EPSILON && fabs(m_vZ[1]) <= FLT_EPSILON) { m_vScale = tm * m_vScale; }
}
