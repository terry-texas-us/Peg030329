...
LRESULT CALLBACK SubProcLPD(HWND hwnd, UINT anMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (anMsg == WM_COMMAND)
    {
        ...
        case ID_OP1:										// Search for an endcap in proximity of current location
            if (lpd::SelEndCapUsingPoint(pView, ptCur, pEndCapSeg, pEndCapMark))
            {	// Endcap located
                char szLen[32]{};
                char szWid[32]{};
                    ...
                    {	// Lead line vector is not null
                        if (lpd::bContSec)
                        {
                            ...
                            else
                            {
                                ...
                                if (trapsegs.Remove(lpd::pEndCapSeg) != 0)
                                {
...
CPnt lpd::GenTakeoff(CPegView* pView, CPnt arPrvPt, CPnt arCurPt, double* adWid, double* adDep, CLine* pLns)
...
bool lpd::GenTap(CPegView* pView, CPnt arPrvPt, CPnt arCurPos, double adWid, double adDep, CLine* pLns)
{	// Generates rectangular tap fitting.
    ...
    double	dSlo = 1.;
    if (detsegs.SelLineUsingPoint(pView, arCurPos) == 0)
        return false;
