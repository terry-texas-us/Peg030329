...
void trap::AddByArea(CPegView* pView, CDC* pDC, CSegs* pSegs, CPnt pt0, CPnt pt1) {
  ...
  if (CVec(pt[0], pt[1]).IsNull()) return;
...
void trap::AddByLn(CPegView* pView, CDC* pDC, CSegs* pSegs, CPnt ptBeg,
                   CPnt ptEnd) {  // Identifies segments which intersect with a line and adds them to the trap.
