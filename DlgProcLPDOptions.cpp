		...
		case WM_COMMAND:
			switch (LOWORD(wParam)) 
			{
				case IDOK:
					...
					lpd::dSecWid = UnitsString_ParseLength(max(app.GetUnits(), Inches), szBuf);
					::GetDlgItemText(hDlg, IDC_DEPTH, szBuf, sizeof(szBuf));
					lpd::dSecDep = UnitsString_ParseLength(max(app.GetUnits(), Inches), szBuf);
					...
					if (IsDlgButtonChecked(hDlg, IDC_LEFT)) {lpd::eJust = lpd::Left;}
					else if (IsDlgButtonChecked(hDlg, IDC_RIGHT)) {lpd::eJust = lpd::Right;}
