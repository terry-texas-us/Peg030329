...
int CLine::DirRelOfPt(const CPnt& pt) const {
  double dDet =
      m_pt[0][0] * (m_pt[1][1] - pt[1]) - m_pt[1][0] * (m_pt[0][1] - pt[1]) + pt[0] * (m_pt[0][1] - m_pt[1][1]);
...
void CLine::Write(CFile& fl) const {
  m_pt[0].Write(fl);
  m_pt[1].Write(fl);
}
  ...
  if (dLen > DBL_EPSILON) {        // Not a zero length line
    if (dX >= std::max(dY, dZ)) {  // Major component of line is along x-axis
      ...
      if (dLen > DBL_EPSILON)                              // Not already on the x-axis
        if (dLen / fabs(pt[0]) < tan(dInfAng * RADIAN)) {  // Within cone of influence .. snap to x-axis
          pt[1] = 0.;
          pt[2] = 0.;
        }
      ...
      if (dLen > DBL_EPSILON)                              // Not already on the y-axis
        if (dLen / fabs(pt[1]) < tan(dInfAng * RADIAN)) {  // Within cone of influence .. snap to y-axis
          pt[0] = 0.;
          pt[2] = 0.;
        }
    } else {
      ...
      if (dLen > DBL_EPSILON)                              // Not already on the z-axis
        if (dLen / fabs(pt[2]) < tan(dInfAng * RADIAN)) {  // Within cone of influence .. snap to z-axis
          pt[0] = 0.;
          pt[1] = 0.;
        }
    }
...
//		false otherwise
bool line::EvalPtProx_xy(const CLine& ln, const CPnt& pt, const double adApert, CPnt& ptProj, double* adRel,
                         double* adDis) {
  ...
  if (*adDis > adApert * adApert) return false;
...
bool line::IntersectionWithPln(const CPnt& ptEnd, const CVec& vDir, const CPnt& ptOnPln, const CVec& vPlnNorm,
                               CPnt* ptInt) {
...
int pFndSwpAngGivPlnAnd3Lns(const CVec& vPlnNorm, const CPnt arP1, const CPnt arP2, const CPnt arP3, const CPnt& ptCP,
                            double* adTheta) {
  double dT[3];
  CPnt rR[3];

  if (arP1 == ptCP || arP2 == ptCP || arP3 == ptCP) return (FALSE);
