...
void CPrimState::SetPen(CDC* pDC, PENCOLOR nPenColor, PENSTYLE nPenStyle) {
  if (CPrim::SpecPenColor() != 0) nPenColor = CPrim::SpecPenColor();

  if (nPenColor == CPrim::PENCOLOR_BYLAYER) nPenColor = WORD(CPrim::LayerPenColor());

  if (CPrim::SpecPenStyle() != 0) nPenStyle = CPrim::SpecPenStyle();

  if (nPenStyle == CPrim::PENSTYLE_BYLAYER) nPenStyle = CPrim::LayerPenStyle();
