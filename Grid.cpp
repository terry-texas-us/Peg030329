...
void DisplayPixel(CPegView* pView, CDC* pDC, COLORREF alRGB, CPnt ptWrld) {
  ...
  if (Pnt4_IsInView(pt)) {  // Point is within current view volume
    CPoint pnt;
...
CPnt grid::UserSnapPt(const CPnt& arPt) {
  ...
  if (IsSnapEnabled()) {
    if (fabs(dXSnapInt) > DBL_EPSILON) {
      ...
      if (fabs(pt[0] - arPt[0]) > dXSnapInt * .5) pt[0] += fsign(dXSnapInt, arPt[0] - ptUsrOrig[0]);
    }
    if (fabs(dYSnapInt) > DBL_EPSILON) {
      pt[1] -= fmod((arPt[1] - ptUsrOrig[1]), dYSnapInt);
      if (fabs(pt[1] - arPt[1]) > dYSnapInt * .5) pt[1] += fsign(dYSnapInt, arPt[1] - ptUsrOrig[1]);
    }
    if (fabs(dZSnapInt) > DBL_EPSILON) {
      pt[2] -= fmod((arPt[2] - ptUsrOrig[2]), dZSnapInt);
      if (fabs(pt[2] - arPt[2]) > dZSnapInt * .5) pt[2] += fsign(dZSnapInt, arPt[2] - ptUsrOrig[2]);
    }
  }
  return (pt);
}
