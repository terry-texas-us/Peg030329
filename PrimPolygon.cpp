...
WORD CPrimPolygon::mS_wEdgeToEvaluate{};
WORD CPrimPolygon::mS_wEdge{};
WORD CPrimPolygon::mS_wPivotVertex{};
...
CPrimPolygon::CPrimPolygon(WORD numberOfPoints, CPnt* points) {
  m_nPenColor = 0;
  m_nIntStyle = POLYGON_SOLID;
  m_nIntStyleId = 0;
  m_wPts = numberOfPoints;
  m_ptOrig = points[0];
  m_vPosXAx = XDIR;
  m_vPosYAx = YDIR;
  m_Pt = new CPnt[m_wPts];

  for (size_t i = 0; i < numberOfPoints; i++) { m_Pt[i] = points[i]; }
}
CPrimPolygon::CPrimPolygon(size_t numberOfPoints, CPnt origin, const CVec& xAxis, const CVec& yAxis,
                           const CPnt* points) {
  m_nPenColor = pstate.PenColor();
  m_nIntStyle = pstate.PolygonIntStyle();
  m_nIntStyleId = pstate.PolygonIntStyleId();
  m_wPts = static_cast<WORD>(numberOfPoints);
  m_ptOrig = origin;
  m_vPosXAx = xAxis;
  m_vPosYAx = yAxis;
  m_Pt = new CPnt[m_wPts];
...
CPrimPolygon::CPrimPolygon(const CPrimPolygon& other) {
  m_nPenColor = other.m_nPenColor;
  m_nIntStyle = other.m_nIntStyle;
  m_nIntStyleId = other.m_nIntStyleId;
  m_ptOrig = other.m_ptOrig;
  m_vPosXAx = other.m_vPosXAx;
  m_vPosYAx = other.m_vPosYAx;
  m_wPts = other.m_wPts;
  m_Pt = new CPnt[m_wPts];
  for (size_t i = 0; i < m_wPts; i++) { m_Pt[i] = other.m_Pt[i]; }
}
const CPrimPolygon& CPrimPolygon::operator=(const CPrimPolygon& other) {
  m_nPenColor = other.m_nPenColor;
  m_nIntStyle = other.m_nIntStyle;
  m_nIntStyleId = other.m_nIntStyleId;
  m_ptOrig = other.m_ptOrig;
...
void CPrimPolygon::Display(CPegView* pView, CDC* pDC) const {
  ...
  if (pDC == 0) {
    ...
    if (m_nIntStyle == POLYGON_HATCH) {
      ...
      DisplayFilAreaHatch(pView, pDC, tm, 1, &iPtLstsId, m_Pt);
    ...
    {       // hollow, solid or pattern
      ...
      for (WORD w = 0; w < m_wPts; w++) { pta[w] = CPnt4(m_Pt[w], 1.); }
      pView->ModelViewTransform(pta);
      Pnt4_ClipPolygon(pta);
      Polygon_Display(pView, pDC, pta);
    }
  }
}
...
CPnt CPrimPolygon::GoToNxtCtrlPt() const {
  if (mS_wPivotVertex >= m_wPts) {  // have not yet rocked to a vertex
    WORD wBeg = WORD(mS_wEdge - 1);
    WORD wEnd = WORD(mS_wEdge % m_wPts);
...
bool CPrimPolygon::SelUsingPoint(CPegView* pView, const CPnt4& pt, double tol, CPnt& ptProj) {
  double dProjDis;

  if (mS_wEdgeToEvaluate > 0 && mS_wEdgeToEvaluate <= m_wPts) {  // Evaluate specified edge of polygon
...
bool CPrimPolygon::SelUsingRect(CPegView* pView, const CPnt& pt1, const CPnt& pt2) {
  CPnts pts;
  for (WORD w = 0; w < m_wPts; w++) pts.Add(m_Pt[w]);
...
bool CPrimPolygon::PvtOnCtrlPt(CPegView* pView, const CPnt4& ptView) const {
  if (mS_wPivotVertex >= m_wPts)
...
bool CPrimPolygon::IsInView(CPegView* pView) const {
  CPnt4 pt[2];
...
bool CPrimPolygon::IsPtACtrlPt(CPegView* pView, const CPnt4& ptPic) const {
  for (WORD w = 0; w < m_wPts; w++) {
    CPnt4 pt(m_Pt[w], 1.);
...
void CPrimPolygon::Read(CFile& fl) {
  CPrim::Read(fl);

  m_nIntStyle = m_nPenStyle;  // note polygon style stuffed up into unused pen style on io
  fl.Read(&m_nIntStyleId, sizeof(short));
  FilePeg_ReadWord(fl, m_wPts);
  m_ptOrig.Read(fl);
  m_vPosXAx.Read(fl);
  m_vPosYAx.Read(fl);

  if (m_Pt != 0) delete[] m_Pt;

  m_Pt = new CPnt[m_wPts];

  for (WORD w = 0; w < m_wPts; w++) m_Pt[w].Read(fl);
}
CPnt CPrimPolygon::SelAtCtrlPt(CPegView* pView, const CPnt4& ptPic) const {
...
void CPrimPolygon::SetHatRefVecs(double dOffAng, double dXScal, double dYScal) {
  ...
  if (vPlnNorm[2] < 0) vPlnNorm = -vPlnNorm;
...
void CPrimPolygon::Transform(const CTMat& tm) {
  ...
  for (WORD w = 0; w < m_wPts; w++) m_Pt[w] = tm * m_Pt[w];
}

void CPrimPolygon::Translate(const CVec& v) {
  m_ptOrig += v;
  for (WORD w = 0; w < m_wPts; w++) m_Pt[w] += v;
}
...
bool CPrimPolygon::Write(CFile& fl) const {
  ...
  for (WORD w = 0; w < m_wPts; w++) m_Pt[w].Write(fl);
...
void DisplayFilAreaHatch(CPegView* pView, CDC* pDC, CTMat& tm, const int iSets, const int* iPtLstsId, CPnt* pta) {
  double dCurStrLen;
  double dEps1;
  double dMaxY;
  double dRemDisToEdg;
  ...
  for (int i0 = 0; i0 < iHatLns; i0++) {
    ...
    for (i = 0; i < iSets; i++) {
      ...
      for (i2 = iBegPt; i2 < (int)iPtLstsId[i]; i2++) {
        ...
        {
          dMaxY = std::max((double)ln[0][1], (double)ln[1][1]);
          iCurEdg = iActEdgs + 1;
  ...
  l1:
    ...
    if (iEndEdg != iBegEdg) {
      ...
      for (i = 1; i <= (iEndEdg - iBegEdg) / 2; i++) {
        ...
        if (lnS[0][0] > edg[iCurEdg].dX) lnS[0][0] -= dTotStrLen;
        iStrId = 0;
