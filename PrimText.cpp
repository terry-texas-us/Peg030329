...
CPnt CPrimText::GetCtrlPt() const { return m_rs.Origin(); }
...
bool CPrimText::SelUsingPoint(CPegView* pView, const CPnt4& ptView, double, CPnt& ptProj) {
  ...
  for (size_t n = 0; n < 4; n++) {
    if (CLine(pt0[n], pt0[(n + 1) % 4]).DirRelOfPt(ptView) < 0) return false;
  }
  ptProj = ptView;
...
void CPrimText::Transform(const CTMat& tm) { m_rs.Transform(tm); }
void CPrimText::TranslateUsingMask(const CVec& v, const DWORD dwMask) {
  if (dwMask != 0) m_rs.SetOrigin(m_rs.Origin() + v);
}
