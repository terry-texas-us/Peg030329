...
CMat4& CMat4::operator*=(const CMat4& mB) {
  ...
  for (int iRow = 0; iRow < 4; iRow++) {
    ...
    for (int iCol = 0; iCol < 4; iCol++) (*this)[iCol][iRow] = vRowA * mB[iCol];
  }
  return *this;
}
...
CMat4& CMat4::Inverse()  // Gauss-Jordan elimination with partial pivoting
{
  ...
  for (int iCol = 0; iCol < 4; iCol++) {  // Find largest pivot in column iCol among rows iCol..3
    ...
    for (i = iCol + 1; i < 4; i++)
      if (fabs(mA[i][iCol]) > fabs(mA[i1][iCol])) i1 = i;
...
CMat4 operator*(const CMat4& mA, const CMat4& mB) {
  ...
  for (int iRow = 0; iRow < 4; iRow++) {
    ...
    for (int iCol = 0; iCol < 4; iCol++) mC[iCol][iRow] = vRowA * mB[iCol];
  }
  return mC;
}
...
bool operator!=(const CMat4& mA, const CMat4& mB) { return !(mA == mB); }
