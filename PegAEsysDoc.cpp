...
CPegDoc::~CPegDoc() {
  ...
  for (WORD w = 0; w < m_PenStyles.GetSize(); w++) delete m_PenStyles[w];
  ...
  if (ar.IsStoring()) {
    if (m_wOpenFileType == FILE_TYPE_DWG || m_wOpenFileType == FILE_TYPE_DXF) {
      ...
      if (m_wOpenFileType == FILE_TYPE_JOB) {
        CFileJob fj;
        if (!fj.OpenForWrite(szFileName)) return;
        fj.WriteHeader();
        fj.WriteSegs(pLayer);
      } else if (m_wOpenFileType == FILE_TYPE_TRACING) {
        CFileTracing ft;
        if (!ft.OpenForWrite(szFileName)) return;
        ft.WriteHeader();
        ft.WriteSegs(pLayer);
      }
...
#pragma tasMSG(TODO : was opening and retaining pFile for locking stuff \
                   .now using the pfile provided in ar which is released automatically)
...
#ifdef _DEBUG
void CPegDoc::AssertValid() const { CDocument::AssertValid(); }

void CPegDoc::Dump(CDumpContext& dc) const { CDocument::Dump(dc); }
#endif  //_DEBUG
...
bool CPegDoc::BlksLookup(CString strKey, CBlock*& pBlock) const {
  if (m_blks.Lookup(strKey, pBlock)) { return true; }
  pBlock = NULL;
  return false;
}
...
void CPegDoc::DoPrimDelete(const CPnt& pt) {
  ...
  if (pSeg != 0) {
    ...
    if (pSeg->GetCount() > 1) {  // remove primitive from segment
      ...
      pSeg = new CSeg(pPrim);
    } else {  // deleting an entire segment
      AnyLayerRemove(pSeg);
      detsegs.Remove(pSeg);

      if (trapsegs.Remove(pSeg) != 0) { app.StatusLineDisplay(TrapCnt); }
    }
    DeletedSegsAddTail(pSeg);
    msgInformation(IDS_MSG_PRIM_ADDED_TO_DEL_SEGS);
  }
}
bool CPegDoc::FileTypeIsTracing(WORD wType) const { return ((wType == FILE_TYPE_TRACING) || (wType == FILE_TYPE_JOB)); }
void CPegDoc::GetExtents(CPnt& ptMin, CPnt& ptMax, const CTMat& tm) const {
  ...
  for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
    ...
    if (pLayer->IsOn()) pLayer->GetExtents(ptMin, ptMax, tm);
  }
}
...
CLayer* CPegDoc::LayersGetAt(int i) const { return (i >= (int)m_layers.GetSize() ? (CLayer*)NULL : m_layers.GetAt(i)); }
...
CLayer* CPegDoc::LayersSelUsingPoint(const CPnt& pt) const {
  ...
  if (pSeg != 0) {
    for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
      CLayer* pLayer = m_layers.GetAt(i);
      if (pLayer->Find(pSeg)) { return (pLayer); }
    }
  }
  for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
    CLayer* pLayer = m_layers.GetAt(i);

    if (pLayer->SelSegUsingPoint(pt) != 0) { return (pLayer); }
  }
  return 0;
}
...
#pragma tasMSG(TODO remove empty notes from blocks)
  ...
  POSITION pos = m_blks.GetStartPosition();
  while (pos != NULL) { m_blks.GetNextAssoc(pos, strKey, pBlock); }
  return (iCount);
}
...
void CPegDoc::SetOpenFile(WORD wFileType, const CString& strFileName) {
  ...
  if (wFileType == FILE_TYPE_NONE) WorkLayerSet(LayersGetAt(0));
...
bool CPegDoc::TracingLoadLayer(const CString& strPathName, CLayer* pLayer) const {
  ...
  if (!FileTypeIsTracing(wFileType)) return false;
...
bool CPegDoc::TracingOpen(const CString& strFileName) {
  ...
  } else {  // create a new layer and append all the segments in the segment file.
...
void CPegDoc::OnClearWorkingLayer() { WorkLayerClear(); }
...
void CPegDoc::OnPrimBreak() {
  ...
  if (pSeg != 0 && detsegs.DetPrim() != 0) {
    ...
    if (pPrim->Is(CPrim::Type::Polyline)) {
      ...
      if (pPolyline->IsLooped()) pSeg->AddTail(new CPrimLine(nPenColor, nPenStyle, pts[pts.GetUpperBound()], pts[0]));
...
void CPegDoc::OnFileQuery() {
  ...
  if (pLayer != 0) {
    ...
    if (iMenu == IDR_LAYER) {
      pMenuSub->CheckMenuItem(ID_LAYER_WORK,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsHot() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_LAYER_ACTIVE,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsWarm() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_LAYER_STATIC,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsCold() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_LAYER_HIDDEN,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsOff() ? MF_CHECKED : MF_UNCHECKED)));
    } else {
      pMenuSub->CheckMenuItem(ID_TRACING_OPEN,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsOpened() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_TRACING_MAP,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsMapped() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_TRACING_VIEW,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsViewed() ? MF_CHECKED : MF_UNCHECKED)));
      pMenuSub->CheckMenuItem(ID_TRACING_CLOAK,
                              static_cast<UINT>(MF_BYCOMMAND | (pLayer->IsOff() ? MF_CHECKED : MF_UNCHECKED)));
    }
...
void CPegDoc::OnLayerMelt() { LayerMelt(m_strIdentifiedLayerName); }
void CPegDoc::OnLayerWork() {
  CLayer* pLayer = LayersGet(m_strIdentifiedLayerName);

  WorkLayerSet(pLayer);
}
void CPegDoc::OnTracingMap() { TracingMap(m_strIdentifiedLayerName); }
void CPegDoc::OnTracingView() { TracingView(m_strIdentifiedLayerName); }
...
void CPegDoc::OnTracingOpen() { TracingOpen(m_strIdentifiedLayerName); }
///<summary>
...
void CPegDoc::OnLayersActiveAll() {
  for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
    CLayer* pLayer = m_layers.GetAt(i);
    if (!pLayer->IsHot()) pLayer->SetStateWarm();
  }
  UpdateAllViews(NULL, 0L, NULL);
}
void CPegDoc::OnLayersStaticAll() {
  for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
    CLayer* pLayer = m_layers.GetAt(i);
    if (!pLayer->IsHot()) pLayer->SetStateCold();
  }
  UpdateAllViews(NULL, 0L, NULL);
}
void CPegDoc::OnLayersRemoveEmpty() { LayersRemoveEmpty(); }
...
void CPegDoc::OnEditTrace() {
  if (::OpenClipboard(NULL)) {
    std::string clipboardFormatName(16, '\0');
      ...
      if (clipboardFormatName == "PegSegs") {
        ...
        if (clipboardData != 0) {
          ...
          if (buffer != nullptr) {
            ...
            mfGetAll(memoryFile, translationVector);
          }
...
void CPegDoc::OnEditTrapPaste() {
  ...
  if (::OpenClipboard(NULL)) {
    ...
    if (IsClipboardFormatAvailable(nClipboardFormat)) {
      if (::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_PASTE_POSITION), pWnd->GetSafeHwnd(), DlgProcSetPasteLoc) >
          0) {
...
void CPegDoc::OnEditTrapWork() {
  if (trapsegs.Identify()) { UpdateAllViews(NULL, HINT_SEGS_SAFE, &trapsegs); }
  trapsegs.RemoveAll();
...
void CPegDoc::OnEditTrapWorkAndActive() {
  ...
  for (int i = 0; i < static_cast<int>(m_layers.GetSize()); i++) {
    CLayer* pLayer = m_layers.GetAt(i);
    if (pLayer->IsHot() || pLayer->IsWarm()) { trapsegs.AddTail(pLayer); }
  }
  UpdateAllViews(NULL, HINT_SEGS_SAFE_TRAP, &trapsegs);
  app.StatusLineDisplay(TrapCnt);
}
void CPegDoc::OnTrapCommandsHighlight() {
  ...
  app.CheckMenuItem(ID_TRAPCOMMANDS_HIGHLIGHT,
                    static_cast<UINT>(MF_BYCOMMAND | (m_bTrapHighlight ? MF_CHECKED : MF_UNCHECKED)));
...
void CPegDoc::OnTrapCommandsCompress() { trapsegs.Compress(); }
void CPegDoc::OnTrapCommandsExpand() { trapsegs.Expand(); }
...
void CPegDoc::OnTrapCommandsSquare() { trapsegs.Square(); }
void CPegDoc::OnTrapCommandsQuery() {
  CWnd* pWnd = AfxGetApp()->GetMainWnd();
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_EDIT_TRAPCOMMANDS_QUERY), pWnd->GetSafeHwnd(),
              DlgProcEditTrap_CommandsQuery);
}
...
void CPegDoc::OnTrapCommandsUnblock() { trapsegs.BreakSegRefs(); }
void CPegDoc::OnSetupPenColor() {
  CWnd* pWnd = AfxGetApp()->GetMainWnd();

  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SETUP_COLOR), pWnd->GetSafeHwnd(),
              reinterpret_cast<DLGPROC>(DlgProcSetupPenColor));
  app.StatusLineDisplay(Pen);
}
void CPegDoc::OnSetupPenStyle() {
  CWnd* pWnd = AfxGetApp()->GetMainWnd();
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SETUP_PENSTYLE), pWnd->GetSafeHwnd(),
              reinterpret_cast<DLGPROC>(DlgProcSetupPenStyle));
  app.StatusLineDisplay(Line);
}
void CPegDoc::OnSetupFillHollow() { pstate.SetPolygonIntStyle(POLYGON_HOLLOW); }
void CPegDoc::OnSetupFillSolid() { pstate.SetPolygonIntStyle(POLYGON_SOLID); }
void CPegDoc::OnSetupFillPattern() {}
void CPegDoc::OnSetupFillHatch() {
  CWnd* pWnd = AfxGetApp()->GetMainWnd();
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SETUP_HATCH), pWnd->GetSafeHwnd(),
              reinterpret_cast<DLGPROC>(DlgProcSetupHatch));
}
...
void CPegDoc::OnSetupNote() {
  CWnd* pWnd = AfxGetApp()->GetMainWnd();
  ::DialogBox(app.GetInstance(), MAKEINTRESOURCE(IDD_SETUP_NOTE), pWnd->GetSafeHwnd(),
              reinterpret_cast<DLGPROC>(DlgProcSetupNote));
}
...
void CPegDoc::OnSegDelete() {
  ...
  if (pSeg != 0) {
    ...
    AnyLayerRemove(pSeg);
    detsegs.Remove(pSeg);
    if (trapsegs.Remove(pSeg) != NULL) app.StatusLineDisplay(TrapCnt);
...
void CPegDoc::OnSegDeleteLast() { detsegs.DelLast(); }
