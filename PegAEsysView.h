...
public:
	...
	void	ModelViewLookAt(const CPnt& ptEye, const CPnt& ptTarget, const CVec& vVwUp) {m_mvActive.LookAt(ptEye, ptTarget, vVwUp); m_mvActive.BuildTransformMatrix();}
	...
	void	ModelViewSetActive(const CModelView& v) {m_mvActive = v; m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetCenteredWnd(double dUExt = 0., double dVExt = 0.) {m_mvActive.SetCenteredWnd(GetAspectRatio(), dUExt, dVExt);}
	void	ModelViewSetDirection(const CVec& v) {m_mvActive.SetDirection(v); m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetEye(const CPnt& pt) {m_mvActive.SetEye(pt); m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetEye(const CVec& v) {m_mvActive.SetEye(v); m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetFarClipDistance(double d) {m_mvActive.SetFarClipDistance(d);  m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetLensLength(double d) {m_mvActive.SetLensLength(d);  m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetMatrix(const CTMat& tm) {m_mvActive.SetMatrix(tm);}
	void	ModelViewSetNearClipDistance(double d) {m_mvActive.SetNearClipDistance(d); m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetPerspectiveEnabled(bool b) {m_mvActive.SetPerspectiveEnabled(b);  m_mvActive.BuildTransformMatrix();}
	void	ModelViewSetTarget(const CPnt& pt) {m_mvActive.SetTarget(pt); m_mvActive.BuildTransformMatrix();}
...
inline CPegDoc* CPegView::GetDocument() const
   { return reinterpret_cast<CPegDoc*>(m_pDocument); }
#endif
