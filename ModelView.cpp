...
void CModelView::AdjustWnd(double dAspectRatio) {
  ...
  if (dUExt <= DBL_EPSILON || dVExt / dUExt > dAspectRatio) {  // Extend width
...
void CModelView::DoTransform(CPnt4s& pta) const {
  ...
  for (int i = 0; i < iPts; i++) { DoTransform(pta[i]); }
}
void CModelView::DoTransform(int iPts, CPnt4* pt) const {
  for (int i = 0; i < iPts; i++) { DoTransform(pt[i]); }
}
...
void CModelView::SetCenteredWnd(
    double dAspectRatio, double dUExt,
    double dVExt) {  // Sets a window which is centered on the view target after adjusting for aspect

  if (dUExt == 0.) dUExt = GetUExt();
  if (dVExt == 0.) dVExt = GetVExt();
...
void CModelView::Translate(const CVec& v) { m_tmView.Translate(v); }
